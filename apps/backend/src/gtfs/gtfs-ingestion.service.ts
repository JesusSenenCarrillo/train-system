...
export class GtfsIngestionService implements OnModuleInit, OnModuleDestroy {
    ...
    async syncNow(): Promise<IngestionSyncStatsDto> {
        ...
        try {
            ...
            const tripIdByCommercialCode = new Map<string, string>();
            for (const snap of normalized.liveSnapshots) {
                if (snap.source === 'LD' && snap.commercialCode && snap.tripId) {
                    tripIdByCommercialCode.set(snap.commercialCode, snap.tripId);
                }
            }

            const ldRoutes = this.extractLdRoutes(feeds.ldFleetWithStations, feeds.ldFleet, tripIdByCommercialCode);
    ...
    private extractLdRoutes(
        ...
        tripIdByCommercialCode: Map<string, string>,
    ): UpsertRouteDto[] {
        ...
        for (const train of ldFleetWithStations.trenes ?? []) {
            ...
            output.push({
                routeKey: key,
                trainId: train.idTren,
                tripId: tripIdByCommercialCode.get(train.idTren) ?? null,
